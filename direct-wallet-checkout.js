(() => {
  ...
  style.textContent = `
    ...
    .nxt-wallet-warning{padding:11px 12px;border:1px solid rgba(251,191,36,.28);border-radius:10px;background:rgba(245,158,11,.075);color:#ffe7a6;font-size:10px;line-height:1.5}.nxt-wallet-actions{display:grid;grid-template-columns:repeat(2,1fr);gap:8px}.nxt-wallet-actions a,.nxt-wallet-actions button{min-height:45px;border-radius:10px;border:1px solid rgba(255,255,255,.105);display:flex;align-items:center;justify-content:center;text-align:center;text-decoration:none;background:#242432;color:#fff;font:850 10px Inter,sans-serif;cursor:pointer;padding:9px;transition:transform .18s,background .18s}.nxt-wallet-actions a:hover,.nxt-wallet-actions button:hover{transform:translateY(-1px);background:#2d2d3d}.nxt-wallet-actions .primary{grid-column:1/-1;min-height:56px;border:0;background:linear-gradient(100deg,#059669,#10b981 55%,#047857);box-shadow:0 12px 27px rgba(5,150,105,.20);font-size:12px}.nxt-wallet-actions [hidden]{display:none}.nxt-wallet-actions [disabled],.nxt-wallet-actions [aria-disabled="true"]{opacity:.45;cursor:not-allowed;pointer-events:none}
    ...
    return new Promise((resolve) => {
      ...
      node.innerHTML = `<div class="nxt-wallet-card"><div class="nxt-wallet-pad">
        ...
        <div class="nxt-wallet-funding-intro">
          <div class="nxt-wallet-funding-intro-head"><span class="nxt-wallet-funding-intro-icon">💳</span><span><strong>Need to buy crypto first?</strong><span>Choose a coin above. The next screen has one clear Card / Apple Pay button that opens the secure Swaps purchase panel while this checkout stays open.</span></span></div>
          ...
          <a class="nxt-wallet-funding-more" href="/shipping-and-payments.html#obtain" target="_blank" rel="noopener noreferrer">Why Swaps.app is recommended + 2 backups ↗</a>
        </div>
  ...
  function renderPayment(quote, context = currentPaymentContext(), initialTxid = '') {
    ...
    const walletButtonLabel = `Open wallet — amount & address filled`;
