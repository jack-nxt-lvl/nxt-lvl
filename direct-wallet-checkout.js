(() => {
  ...
  function chooseAsset() {
    return new Promise((resolve) => {
      ...
      node.innerHTML = `<div class="nxt-wallet-card"><div class="nxt-wallet-pad">
        ...
        <p>Send crypto from your wallet directly to the matching NXT LVL receiving address. No payment processor holds your funds.</p>
  ...
  async function openSwapsFunding(quote, button, status, fallbackLink) {
    ...
    layer.innerHTML = `<button type="button" class="nxt-swaps-backdrop" aria-label="Close Swaps panel"></button><aside class="nxt-swaps-drawer" role="dialog" aria-modal="true" aria-label="Buy ${escapeHtml(quote.asset)} with card or Apple Pay">
      ...
      <div class="nxt-swaps-frame-wrap"><div class="nxt-swaps-loading" role="status">Loading secure Swaps purchase options…</div><iframe class="nxt-swaps-frame" data-swaps-frame title="Swaps crypto purchase" allow="payment; clipboard-read; clipboard-write" referrerpolicy="strict-origin-when-cross-origin"></iframe></div>
      <div class="nxt-swaps-footer"><span>NXT LVL never sees or stores your card information. Swaps and its providers control availability, fees, limits, and identity checks.</span><a href="${escapeHtml(url)}" target="_blank" rel="noopener noreferrer">If the panel cannot load, open Swaps separately ↗</a></div>
    </aside>`;
    document.body.appendChild(layer);
    activeFundingDrawer = layer;

    const frame = layer.querySelector('[data-swaps-frame]');
    const loading = layer.querySelector('.nxt-swaps-loading');
