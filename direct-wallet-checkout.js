    ...
    overlay.innerHTML = `<div class="nxt-wallet-card nxt-wallet-pay" data-asset="${escapeHtml(quote.asset)}" role="dialog" aria-modal="true" aria-label="Direct ${escapeHtml(quote.asset)} payment">
      ...
      <div class="nxt-wallet-body">
        ...
        <div class="nxt-wallet-order">
          ...
          <div class="nxt-wallet-existing"><div><b>Choose the easiest way for you</b><span>Already have ${escapeHtml(quote.asset)}? Wallet payment is fastest. Need it first? Use the card / Apple Pay option.</span></div><em>2 clear choices</em></div>
          <div class="nxt-wallet-actions"><a class="primary" data-open-wallet href="${escapeHtml(quote.paymentUri)}">I have crypto — open my wallet</a><button type="button" class="buy" data-buy-crypto>I need crypto — Card / Apple Pay</button><button type="button" data-browser-pay ${quote.asset === 'BTC' ? 'hidden' : ''}>Use browser wallet</button><button type="button" data-copy-all>Copy payment details</button></div>
