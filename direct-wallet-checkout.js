(() => {
  ...
  style.textContent = `
    ...
    @media(max-width:700px){.nxt-wallet-pad{padding:22px 16px}.nxt-wallet-card h2{font-size:23px}.nxt-wallet-trust{grid-template-columns:1fr}.nxt-wallet-pay{height:96vh}.nxt-wallet-head{padding:14px}.nxt-wallet-headcoin{width:38px;height:38px;flex-basis:38px}.nxt-wallet-secure{display:none}.nxt-wallet-head-copy strong{font-size:14px}.nxt-wallet-body{grid-template-columns:1fr;padding:16px}.nxt-wallet-qr{width:210px;margin:auto}.nxt-wallet-buy-head{align-items:flex-start}.nxt-wallet-buy-badge{font-size:7px}.nxt-wallet-buy-confidence{display:grid;grid-template-columns:1fr}.nxt-wallet-buy-steps{grid-template-columns:1fr}.nxt-wallet-buy-step{display:grid;grid-template-columns:24px 1fr;align-items:center;gap:8px}.nxt-wallet-buy-step b{margin:0}.nxt-wallet-buy-step small{grid-column:2}.nxt-wallet-buy-tools{grid-template-columns:1fr}.nxt-wallet-buy-copy{min-height:42px}.nxt-wallet-actions{grid-template-columns:1fr}.nxt-wallet-verifyrow{grid-template-columns:1fr}.nxt-wallet-verify button{min-height:42px}}
    @media(prefers-reduced-motion:reduce){.nxt-wallet-buy-main,.nxt-wallet-buy-main:before,.nxt-wallet-close,.nxt-wallet-actions a,.nxt-wallet-actions button{transition:none!important}}
  `;
  ...
  async function openSwapsFunding(quote, button, status, fallbackLink) {
    ...
    const popup = window.open('', '_blank', helper.popupFeatures(window.screen, window));
    if (!popup) {
      fallbackLink.href = url;
      fallbackLink.hidden = false;
      setSwapsStatus(status, 'Your browser blocked the side window. Use the safe button below instead; this checkout will stay open in its original tab.', true);
      return;
    }
    try {
      popup.opener = null;
    ...
    button.textContent = 'Secure Swaps window opened ✓';
    const copied = await copyForSwaps(quote.address);
    const network = quote.asset === 'USDT' ? ' and choose Ethereum ERC-20' : '';
    const returnStep = quote.asset === 'ETH'
      ? 'After the purchase, return here and paste the transaction ID from Swaps below.'
      : 'Keep this checkout open. It will detect the incoming payment automatically.';
    setSwapsStatus(
      status,
      copied
        ? `Address copied ✓ In Swaps, choose an available card or Apple Pay option, paste the copied address${network}, and confirm the coin and amount. ${returnStep}`
        : `In Swaps, choose an available card or Apple Pay option. Press “Copy address” here, paste it into Swaps${network}, and confirm the coin and amount. ${returnStep}`,
    );
    setTimeout(() => {
      button.disabled = false;
      button.textContent = `Reopen Buy ${quote.asset} window ↗`;
    }, 1800);
  }
  ...
  function renderPayment(quote, context = currentPaymentContext(), initialTxid = '') {
    ...
    overlay.innerHTML = `<div class="nxt-wallet-card nxt-wallet-pay" data-asset="${escapeHtml(quote.asset)}" role="dialog" aria-modal="true" aria-label="Direct ${escapeHtml(quote.asset)} payment">
      ...
      <div class="nxt-wallet-body">
        ...
        <div class="nxt-wallet-order">
          ...
          <div class="nxt-wallet-buy"><div class="nxt-wallet-buy-head"><div><span>No crypto yet? Buy it here</span><small>Open a secure purchase window powered by the legitimate swaps.app marketplace</small></div><div class="nxt-wallet-buy-badge">Step by step</div></div><div class="nxt-wallet-buy-confidence"><span>✓ Direct crypto checkout stays open</span><span>✓ Address copied automatically</span><span>✓ Card details stay with the provider</span></div><button type="button" class="nxt-wallet-buy-main" data-buy-crypto>Start — Buy ${escapeHtml(quote.asset)} Here ↗</button><div class="nxt-wallet-buy-steps"><div class="nxt-wallet-buy-step"><b>1</b>Press Start above<small>A secure window opens with ${escapeHtml(quote.asset)} and the requested amount filled in</small></div><div class="nxt-wallet-buy-step"><b>2</b>Choose how to pay<small>Select an available debit card, credit card, or Apple Pay option and complete any provider checks</small></div><div class="nxt-wallet-buy-step"><b>3</b>${escapeHtml(fundingNetworkStep)}<small>${escapeHtml(fundingNetworkDetail)}</small></div><div class="nxt-wallet-buy-step"><b>4</b>${escapeHtml(fundingReturnStep)}<small>${escapeHtml(fundingReturnDetail)}</small></div></div><div class="nxt-wallet-buy-tools"><span><b>Receiving address backup</b>It copies automatically. Use this only if you need to copy it again.</span><button type="button" class="nxt-wallet-buy-copy" data-copy-for-swaps>Copy address</button></div><small class="nxt-wallet-buy-note">${escapeHtml(funding.title)} ${escapeHtml(funding.detail)} <b>Before paying, confirm the coin, network, amount, and address match this checkout.</b></small><div class="nxt-wallet-buy-status" data-buy-status role="status" aria-live="polite"></div><a class="nxt-wallet-buy-fallback" data-swaps-fallback target="_blank" rel="noopener noreferrer" hidden>Open secure Swaps page in a new tab ↗</a></div>
    ...
    swapsFallback.onclick = () => {
      copyForSwaps(quote.address).then((copied) => {
        if (copied) setSwapsStatus(buyStatus, 'Address copied ✓ Swaps is opening in a new tab. Return to this checkout after the purchase.');
      });
    };
