...
const { buildQuantityPricing } = require('../quantity-pricing.js');

// These products are appended to the browser catalog by storefront enhancement
// scripts. Keep their accepted IDs, labels, and prices in the trusted server
// catalog as well so a visible product can never become unpayable at checkout.
const STOREFRONT_SUPPLEMENTAL_COMPOUNDS = Object.freeze([
  {
    id: 'cjc1295-dac-10',
    name: 'CJC-1295 with DAC — 10mg',
    pricing: [
      { label: '1 Vial', price: 89.99 }, { label: '2 Vials', price: 179.98 },
      { label: '3 Vials', price: 269.97 }, { label: '4 Vials', price: 359.96 },
  ...
  },
  {
    ...
    name: 'PT-141 — 10mg',
    pricing: [
      { label: '1 Vial', price: 45 }, { label: '5 Vials', price: 184.5 },
      { label: '10 Vials', price: 315 },
    ],
  },
  {
    id: 'bac-water-10ml',
    ...
    pricing: [
      { label: '1 Vial', price: 10 }, { label: '5 Vials', price: 41 },
      { label: '10 Vials', price: 70 },
    ],
  },
  {
    id: 'glutathione-1500',
    ...
    pricing: [
      { label: '1 Vial', price: 79 }, { label: '5 Vials', price: 324 },
      { label: '10 Vials', price: 553 },
    ],
  },
  {
    id: 'slu-pp-332-10',
    ...
    pricing: [
      { label: '1 Vial', price: 85 }, { label: '5 Vials', price: 348.5 },
      { label: '10 Vials', price: 595 },
    ],
  },
]);

const SUPPLEMENTAL_FREEZE_DRIED_IDS = new Set([
  'cjc1295-dac-10',
  'cjc1295-no-dac-10',
  'pt141-10',
  'bac-water-10ml',
...
const TRUSTED_COMPOUNDS_BY_ID = new Map(
  [...compounds, ...STOREFRONT_SUPPLEMENTAL_COMPOUNDS]
    .map((product) => {
      const category = product.category || (SUPPLEMENTAL_FREEZE_DRIED_IDS.has(product.id) ? 'freeze-dried' : '');
      const trustedProduct = {
        ...product,
        category,
        pricing: buildQuantityPricing(product, category),
      };
      return [trustedProduct.id, trustedProduct];
    }),
);
...
function normalizeOrder(items, fulfillment) {
  ...
  const normalizedItems = items.map((item) => {
    ...
    const product = TRUSTED_COMPOUNDS_BY_ID.get(productId);
