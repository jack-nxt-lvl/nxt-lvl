...
function fundingGuidance(country) {
  ...
  return {
    methods,
    title: `Need crypto first? ${methods.join(', ').replace(/, ([^,]*)$/, ' or $1')} may be available inside a compatible wallet app.`,
    detail: 'Availability, fees, limits, and identity checks are controlled by the wallet or exchange. NXT LVL accepts crypto only and does not process card payments.',
  };
}
