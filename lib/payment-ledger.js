...
local reviewOrder = redis.call('GET', KEYS[3])
...
if reviewOrder and reviewOrder ~= ARGV[1] then
  return {'TX_USED', reviewOrder}
end
...
const FLAG_PAYMENT_REVIEW_SCRIPT = `
local txOrder = redis.call('GET', KEYS[1])
local orderTx = redis.call('GET', KEYS[2])
local reviewOrder = redis.call('GET', KEYS[3])
if txOrder then
  if txOrder ~= ARGV[1] then
    return {'TX_USED', txOrder}
  end
  return {'ORDER_ALREADY_PAID', txOrder}
end
if orderTx then
  return {'ORDER_ALREADY_PAID', orderTx}
end
...
if reviewOrder then
  return {'IDEMPOTENT_REVIEW', ARGV[1]}
end
return {'FLAGGED', ARGV[1]}
`;
...
function paymentKeys(asset, txid, orderId) {
  ...
  return {
    ...
    txReview: `nxt:payment:${KEY_VERSION}:{claims}:review:tx:${txRef}`,
    orderReview: `nxt:payment:${KEY_VERSION}:{claims}:review:order:${order}`,
  };
}
...
async function claimPayment(asset, txid, orderId) {
  const keys = paymentKeys(asset, txid, orderId);
  const result = await redisCommand([
    'EVAL', CLAIM_PAYMENT_SCRIPT, '3', keys.txClaim, keys.orderClaim, keys.txReview, keys.order, keys.txRef,
  ]);
...
async function flagPaymentForReview(asset, txid, orderId) {
  const keys = paymentKeys(asset, txid, orderId);
  const result = await redisCommand([
    'EVAL', FLAG_PAYMENT_REVIEW_SCRIPT, '4',
    keys.txClaim, keys.orderClaim, keys.txReview, keys.orderReview,
    keys.order, keys.txRef,
  ]);
  if (!Array.isArray(result) || !result[0]) {
    throw new PaymentLedgerError('The secure payment ledger returned an invalid review response.');
  }
  return { status: String(result[0]), existing: result[1] ? String(result[1]) : null, durable: true };
}
