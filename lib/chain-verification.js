...
const DEFAULT_ETHEREUM_RPCS = ['https://ethereum-rpc.publicnode.com', 'https://eth.drpc.org'];
...
function ethereumLogRpcUrls() {
  return uniqueUrls([
    ...
    process.env.ETHEREUM_BACKUP_RPC_URL,
    DEFAULT_ETHEREUM_RPCS[1],
    process.env.ETHEREUM_RPC_URL,
    DEFAULT_ETHEREUM_RPCS[0],
  ]).slice(0, 3);
}
...
function inspectBitcoinTransaction(tx, tipHeight, quote, canonicalBlockHash) {
  ...
  if (!amount.ok) return { ...amount, confirmations: 0, requiredConfirmations: minimum, replaceable };
...
function inspectEthereumTransaction(tx, receipt, block, currentBlockHex, quote, finalizedBlockHex) {
  ...
  const amount = compareAmount(received, quote.amountUnits);
  if (!amount.ok) return { ...amount, confirmations: 0, requiredConfirmations: minimum };
...
async function verifyEthereum(txid, quote) {
  const checks = await Promise.all(ethereumRpcUrls().map((url) => verifyEthereumAtRpc(url, txid, quote)));
  if (checks.every((check) => check.status === 'not_found')) {
...
async function findUsdtPaymentAtRpc(quote, rpcUrl) {
  ...
  const finalizedBlock = await rpcAt(rpcUrl, 'eth_getBlockByNumber', ['finalized', false]);
  if (!finalizedBlock || !finalizedBlock.number) throw new Error('The Ethereum RPC does not expose finalized blocks.');
  const currentBlock = BigInt(finalizedBlock.number);
