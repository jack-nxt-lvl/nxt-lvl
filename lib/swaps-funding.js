(function exposeSwapsFunding(root, factory) {
  ...
  function popupFeatures(screenInfo = {}, windowInfo = {}) {
    const availableWidth = Math.max(360, Number(screenInfo.availWidth) || Number(windowInfo.outerWidth) || 1280);
    const availableHeight = Math.max(640, Number(screenInfo.availHeight) || Number(windowInfo.outerHeight) || 800);
    const width = Math.min(520, Math.max(390, Math.round(availableWidth * 0.36)));
    const height = Math.min(840, Math.max(640, availableHeight - 36));
    ...
    const left = Math.max(screenLeft, screenLeft + availableWidth - width - 12);
